...
def mariaConnector():
    global connected, conn, cur, user, password, host, port, database, Formatted
    ...
    while connected and not dbdefined and not formatReady:
        ...
        databQuery = input('')
        if databQuery == 'y':
            print("Here is a list of the databases.")
            ...
            while True:
                ...
                dbinput = input('')
                if any(str(dbinput) in s for s in dblist):
                    database = dbinput
                    print("\nAwesome, attempting to connect.\n.\n.")
                    try:
                        conn = mariadb.connect(
                            user=user,
                            password=password,
                        ...
                        cur = conn.cursor()
                        #attempts connecting to the server with the provided database, if you couldn't it would throw an exception.
                        cur.execute("SHOW DATABASES;")
                        dblist = cur.fetchall()
                        print("Successfully connected to the database.")
                        #checks if the database is formatted or not.
                        #program first checks if the tables exists
                        cur.execute("SHOW TABLES;")
                        tableList = cur.fetchall()
                        #if the tables exist, the database must be formatted.
                        if any("sessions" in s for s in tableList) & any("accounts" in s for s in tableList):
                            print("The program has detected that the database is already correctly formatted for use by the website.")
                            dbdefined, Formatted = True, True
                        #if the server returned "None", the database is empty, therefore ready to be formatted.
                        elif len(tableList) == 0:
                            dbdefined, formatReady = True, True
                        #There is stuff in the server.
                        else:
                            print("The server detected that there is content in the database provided. Please empty the database before continuing..")
                            print(tableList)
                            sleep(7)
                            sys.exit()
                    except Exception as e:
                        ...
                        continue
                elif not any(str(dbinput) in s for s in dblist):
            ...
            while True:
                ...
                print(dblist)
                dbname = input('\n')
                if not any(str(dbinput) in s for s in dblist):
                ...
                elif any(str(dbinput) in s for s in dblist):
                    print("Name " + str(dbname) + " already used, please try another name that is not in the list. Restarting...\n\n")
                    continue
    #this is when the connection credentials are valid, the database has been defined, and is empty... Meaning it's ready to be formatted.
    while connected and dbdefined and formatReady and not Formatted:
